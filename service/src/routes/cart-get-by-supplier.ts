...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      const cart = await cartRepo.selectOne({ supplierId: req.query.supplierId, merchantId: req.query.merchantId, userId: req.currentUser?.id, status: { $in: [CartStatus.Created, CartStatus.Pending, CartStatus.Returned] } });
      if (cart) {
